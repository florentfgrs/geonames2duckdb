...
from toolbelt.duckdb_database_handling import database_exists, execute_query_on_db, create_database
...
with tempfile.TemporaryDirectory() as temp_dir:
    ...
    logging.info(f"Temporary directory is using for download and unzip data: {str(temp_path)}")
    ...
    with open("./sql/table_geonames.sql", "r") as file:
        sql_query = file.read()
        sql_query = sql_query.format(csv_path=str(temp_path / "allCountries.txt"))
        execute_query_on_db(sql=sql_query, db_path=database_path, spatial=True, query_name="Insert geonames from txt to database table")
